#!/bin/bash
# mcp-tg MCP Server - One-line Installation Script
# Usage: curl -sSL https://raw.exampleusercontent.com/local/mcp-tg/main/install.sh | bash
...
# Configuration
REPO_URL="https://example.com/local/mcp-tg"
INSTALL_DIR="$HOME/.local/share/mcp-tg"
BIN_DIR="$HOME/.local/bin"
CONFIG_DIR="$HOME/.config/mcp-tg"

# Functions
print_header() {
    echo -e "${BLUE}============================================${NC}"
    echo -e "${BLUE}    mcp-tg MCP Server Installation${NC}"
    echo -e "${BLUE}============================================${NC}"
    echo ""
}
...
download_and_install() {
    ...
    if [ "$HAS_GIT" = true ]; then
        ...
        LATEST_RELEASE=$(curl -s https://api.example.com/repos/local/mcp-tg/releases/latest | grep "tarball_url" | cut -d '"' -f 4)
    ...
    cat > "$BIN_DIR/mcp-tg" << 'EOF'
#!/usr/bin/env python3
import sys
import os
sys.path.insert(0, os.path.expanduser("~/.local/share/mcp-tg"))
from mcp_telegram.cli import main
if __name__ == "__main__":
    main()
EOF
    chmod +x "$BIN_DIR/mcp-tg"
...
setup_configuration() {
    ...
    if [[ ":$PATH:" != *":$BIN_DIR:"* ]]; then
        ...
        echo "" >> "$SHELL_RC"
        echo "# mcp-tg MCP Server" >> "$SHELL_RC"
...
verify_installation() {
    ...
    if command -v mcp-tg &> /dev/null; then
        print_success "mcp-tg command is available"
        
        # Test import
        if python3 -c "import mcp_telegram" 2>/dev/null; then
        ...
        print_error "mcp-tg command not found"
...
print_next_steps() {
    ...
    echo "3. Run first authentication: ${BLUE}mcp-tg sign-in${NC}"
    echo "4. Start the MCP server: ${BLUE}mcp-tg run${NC}"
    echo ""
    if [ "$HAS_DOCKER" = true ]; then
